...
def tf_idf(tf, df, doc_id):
    log_value = math.log(no_of_docs / df)
        ...
        for i in range(len(data)):
            ...
            if query_word not in stopwords:
                ...
                for hit in result['hits']['hits']:
                    ...
                    if tf > 0:
                        if doc_no in tf_idf_dict:
                            tf_idf_dict[doc_no] = tf_idf_dict[doc_no] + tf_idf(tf, df, doc_no)
                        else:
                            tf_idf_dict[doc_no] = tf_idf(tf, df, doc_no)

        sorted_tfidf_dict = (sorted(tf_idf_dict.iteritems(), key=lambda x: -x[1])[:1000])
        output = open('tfidf_dict_pkl.pkl', 'wb')
        pickle.dump(sorted_tfidf_dict, output)
